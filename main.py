...
def send_kakao_message(notice, access_token):
    """
...
def main():
    ...
    # 3. 메시지 전송
    if not notices:
        print("결과: 새로운 공지사항이 없습니다.")
    else:
        print(f"결과: {len(notices)}개의 새로운 공지사항을 찾았습니다.")
        for notice in notices:
            send_kakao_message(notice, new_access_token)
